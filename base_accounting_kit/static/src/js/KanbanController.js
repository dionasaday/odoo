        ...
        if (currentStLineId !== stLineId) {
            ...
            try {
                ...
                if (data && data.length > 0 && data[0].lines_widget_json) {
                    const jsonString = data[0].lines_widget_json;
                    // Validate that jsonString is a string and not empty
                    if (typeof jsonString !== 'string' || jsonString.trim() === '') {
                        console.warn("Invalid lines_widget_json: not a string or empty");
                        return;
                    }
                    ...
                    try {
                        ...
                        if (!parsedData || typeof parsedData !== 'object' || Array.isArray(parsedData)) {
                            console.warn("Invalid parsed JSON data: not an object");
                            return;
                        }
                        
                        // Safely extract move_id
                        if (parsedData.move_id && typeof parsedData.move_id === 'string') {
                            const moveIdMatch = parsedData.move_id.match(/\((\d+),\)/);
                            parsedData.numeric_move_id = moveIdMatch ? parseInt(moveIdMatch[1]) : null;
                        } else {
                            parsedData.numeric_move_id = null;
                        }
                        this.state.moveLineData = parsedData;
                    } catch (parseError) {
                        console.error("Error parsing lines_widget_json:", parseError);
